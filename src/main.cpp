...
#if defined(NDEBUG)
# error "Wbcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x85ad17ccdb0590d4e1e86d693018c2b4fc1ff9560bc29f0ac202a1ced7a92a28");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wbcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wbcoin Signed Message:\n";
...
{
    // Wbcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Wbcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wbcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Wbcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wbcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wbcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbd2158e3f30c7bb1d183b03907b8541f0f5656cc25aa7558497e3d997353484a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "WbCoin is the Next";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a66545757415101311cf6dfc506ddb304bc3e8be65a4e9221033dd776d829c75759b31630d773ca5de524ed9ed7cd3b04451860a520d6773ccf7d81c1b8a2418") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517112258;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084674802;

        if (fTestNet)
        {
            block.nTime    = 1517112213;
            block.nNonce   = 386367419;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbe05f113f0b9f029b4fba576c98d686616fd59e01171c011495b9ba5ee0d779a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Wbcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WbcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WbcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WbcoinMiner(CWallet *pwallet)
{
    printf("WbcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wbcoin-miner");
    ...
    try { loop {
        ...
        printf("Running WbcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WbcoinMiner, pwallet));
}
